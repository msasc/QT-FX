...
package com.qtfx.library.gui.old;
...
import com.qtfx.library.task.old.Task;
...
public class TaskPane extends Pane {
	...
	private void layoutComponents() {
		...
		buttonInfo.setOnAction((EventHandler<ActionEvent>) e -> {
			if (task instanceof TaskPool) {
				TaskPool pool = (TaskPool) task;
				List<Task> tasks = pool.getTasks();
				for (Task t : tasks) {
					if (t.getException() != null) {
						StringBuilder title = new StringBuilder(task.getTitle());
						if (t.getTitle() != null && !t.getTitle().isEmpty()) {
							title.append(" (");
							title.append(t.getTitle());
							title.append(")");
						}
						Alert.error(title.toString(), getStackTrace(t.getException()));
						break;
					}
				}
			} else {
				Alert.error(task.getTitle(), getStackTrace(task.getException()));
			}
		...
		task.stateProperty().addListener((observable, oldValue, newValue) -> {
			...
			// A task pool. If sub-task has failed, the pool cancels.
			if (task instanceof TaskPool) {
				TaskPool pool = (TaskPool) task;
				List<Task> tasks = pool.getTasks();
				for (Task t : tasks) {
					if (t.getState().equals(State.FAILED)) {
						buttonInfo.setDisable(false);
						labelState.textProperty().bind(new SimpleStringProperty("State: ").concat(t.stateProperty().asString()));
						break;
					}
				}
			}
