...
public class RSA {
	 ...
	 */
	public static final BigInteger GENERATED_P =
		new BigInteger(
	 ...
	 */
	public static final BigInteger GENERATED_Q =
		new BigInteger(
	 ...
	 */
	private static final int GROUP_BYTES_SIZE = 127;
	 ...
	 * easily descriptable keys. The argument is the phi Euler function.
	 * 
	 * @param phi The phi Euler function to find the relative prime.
	...
	/**
	 * Constructs a new RSA assign two large prime numbers.
	 * 
	...
	public String encrypt(String decryptedString) {
		...
		for (byte enc : encrypted) {
			b.append(Numbers.toHexString(enc));
		}
		return b.toString();
	}
	...
	public String decrypt(String encryptedString) {
		if (!Numbers.isEven(encryptedString.length())) {
		...
		for (int i = 0; i < encrypted.length; i++) {
			...
			encrypted[i] = Numbers.parseByte(b);
			cursor += 2;
		}
	...
	private List<BigInteger> groupByteArray(byte[] bytes) {
		...
		while (cursor < bytes.length) {
			byte[] bigIntegerBytes = new byte[Math.min(GROUP_BYTES_SIZE, bytes.length - cursor)];
